...
const TaskPage: React.FC = () => {
  ...
  const { data, loading } = useQuery<{ task: Task }>(GET_TASK_BY_ID, {
  ...
  const handleDeleteTask = (e: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
    e.preventDefault();
    deleteTask({ variables: { id: task_id } }).then(() => {
      navigate(`/`);
      ...
      <div className="relative w-full p-4 overflow-hidden bg-white shadow-lg rounded-xl md:w-72 dark:bg-gray-800">
        ...
        <div className="items-center justify-between my-2 mt-5">
          ...
          <div className="edit-and-delete mt-5 flex">
            ...
            <Link to="/" onClick={handleDeleteTask} className="ml-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
