      ...
      <form className="flex w-full max-w-sm space-x-3" onSubmit={handleSubmit}>
        <div className="w-full max-w-2xl px-5 py-10 m-auto mt-10 bg-white rounded-lg shadow dark:bg-gray-800">
          <div className="mb-6 text-3xl font-bold text-center text-gray-800 dark:text-white">
            Editing Task
            <p className="text-gray-400 font-semibold text-2xl">{title}</p>
          </div>
          <div className="grid max-w-xl grid-cols-2 gap-4 m-auto">
            ...
            <div className="col-span-2 text-right">
              <button
                ...
                type="submit"
                className={`py-2 px-4 ${title.length === 0 || date == null ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'} focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg`}
              >
                Submit
              </button>
            </div>
